...
def jaccard_score(set_a: set, set_b: set):
	return len(set_a.intersection(set_b)) / len(set_a.union(set_b))

def get_instance_metrics(g_clusters, p_clusters):
	mat_jac = np.zeros((len(g_clusters), len(p_clusters)))

	for i, g in tqdm(enumerate(g_clusters)):
		for j, p in enumerate(p_clusters):
			jac = jaccard_score(g, p)
			mat_jac[i][j] = jac

	jac_for_g_max_p = mat_jac.max(1).mean()
	jac_for_p_max_g = mat_jac.max(0).mean()
	sor_for_g_max_p = (2 * jac_for_g_max_p) / (1 + jac_for_g_max_p)
	sor_for_p_max_g = (2 * jac_for_p_max_g) / (1 + jac_for_p_max_g)

	return {
		'jac_for_g_max_p': jac_for_g_max_p,
		'jac_for_p_max_g':jac_for_p_max_g,
		'sor_for_g_max_p': sor_for_g_max_p,
		'sor_for_p_max_g': sor_for_p_max_g,
	}
...
def pairwiseMetric(C_clust2ele, E_ele2clust, E_clust2ent):
	...
	for _, cluster in C_clust2ele.items():
		# if len(cluster) > 1000:
		# 	print("cluster {} too much, continue".format(list(cluster)[:30]))
		# 	continue
		all_pairs = list(itertools.combinations(cluster, 2))
...
def get_metrics(C_ele2clust, C_clust2ele, E_ele2clust, E_clust2ent):
	...
	return {
		...
		'n_gold': len(E_clust2ent),
		'n_pred': len(C_clust2ele),
